...
public class BombTest {
     ...
     */
    private Bomb testBomb = new Bomb(PlayerID.PLAYER_1, new Cell(0, 0),
            Sq.constant(2), 2);
    ...
    public void constructor2SetsFuseLengths() {
        // 3, 2, 1
        Sq<Integer> fuseLengthsModel = Sq.constant(3).limit(1)
                .concat(Sq.constant(2).limit(1))
                .concat(Sq.constant(1).limit(1));
            ...
            for (int j = 0; j < Ticks.EXPLOSION_TICKS; ++j) {
                ...
                for (int k = 0; k < range; ++k) {
                    if (k == 0)
                        ...
                        assertEquals(bomb.position().neighbor(dir),
                                particle.head());
