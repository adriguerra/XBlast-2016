...
public class PlayerTest {
    ...
    private List<State> states = Arrays.asList(State.INVULNERABLE,
            State.VULNERABLE, State.DYING, State.DEAD);
    ...
    public void setUp() {
        ...
        lifeStates = Sq.constant(new LifeState(lives,
                LifeState.State.VULNERABLE));
        directedPos = DirectedPosition.stopped(new DirectedPosition(
                subCellPosition, direction));
    ...
    /**
     * Helper function to skip `LifeState` in a Sq while a predicate is true. In
     * case of constant Sq, signal it with an empty Optional.
     *
     * By skipping elements, we don't penalize if the sequence was not
     * implemented with the right length and allows us to test what we really
     * want.
     *
     * @param lifeStates
     *            the Sq that will be skipped
     * @param skip
     *            the predicate: while it succeeds, we skip.
     * @return The skipped Sq of LifeState wrapped in a Optional or an empty
     *         Optional in case the Sq is thought to be constant
     */
    private Optional<Sq<LifeState>> skipSqWhile(Sq<LifeState> lifeStates,
            Predicate<State> skip) {
        Sq<LifeState> ls = lifeStates;

        int ticks = 0; // used to avoid an infinite loop in case of a constant
                       // Sq
        while (ticks < THRESHOLD && !ls.isEmpty()
                && skip.test(ls.head().state())) {
            ls = ls.tail();
            ++ticks;
        }
    ...
    public void canMoveWorksForRightStates() {
        for (State state : states) {
            ...
            boolean expected = state == State.INVULNERABLE
    ...
    public void statesForNextLifeShouldHaveAPeriodOfDyingState() {
        ...
        for (int tick = 0; tick < Ticks.PLAYER_DYING_TICKS; ++tick) {
            ...
            assertEquals("State should be DYING, not " + ls.state(),
                    State.DYING, ls.state());
            lifeStates = lifeStates.tail();
        }

        assertTrue(
                "State should be different than DYING after PLAYER_DYING_DICKS",
    ...
    public void statesForNextLifeShouldHaveOneLessLifeAFTERDyingPeriod() {
        int livesAmount = 3;
        Player player = new Player(id, livesAmount, cellPosition, maxBombs,
                bombRange);
        Sq<LifeState> lifeStates = player.statesForNextLife();

        assertEquals(
                "The DYING period should have the same amount of life than at the begining",
                livesAmount, lifeStates.head().lives());

        // Skip DYING state
        Optional<Sq<LifeState>> maybeSkipped = skipSqWhile(lifeStates,
                (s) -> s == State.DYING);
        ...
        assertEquals(
                "After the dying period, lifeState should have one less life",
    ...
    public void statesForNextLifeWithOneLifeShouldBeDeath() {
        ...
        Optional<Sq<LifeState>> maybeSkipped = skipSqWhile(lifeStates,
                (s) -> s == State.DYING);
        assertTrue(
                "The sequence was a State.DYING constant when it shouldn't.",
                maybeSkipped.isPresent());
        lifeStates = maybeSkipped.get();
    ...
    public void statesForNextLifeShouldHaveAnInvulnerablePeriod() {
        ...
        for (int tick = 0; tick < Ticks.PLAYER_INVULNERABLE_TICKS; ++tick) {
            assertEquals("State should be INVULNERABLE for tick 0 to "
                    ...
                    + " but faded at tick " + tick, State.INVULNERABLE,
                    lifeStates.head().state());
            lifeStates = lifeStates.tail();
        }

        assertTrue(
                "State should not be INVULNERABLE after the invulnerability period faded.",
    ...
    public void statesForNextLifeShouldHaveAnVulnerablePeriodAfterInvulnerabilty() {
        ...
        Optional<Sq<LifeState>> maybeSkipped = skipSqWhile(lifeStates,
                (s) -> s != State.INVULNERABLE);

        // Skip the invulnerability period
        maybeSkipped = maybeSkipped.flatMap((lstates) -> skipSqWhile(lstates, (
                s) -> s == State.INVULNERABLE));

        assertTrue(
                "The LifeState does not have or does not escape the INVULNERABLE state",
                maybeSkipped.isPresent());
        lifeStates = maybeSkipped.get();
    ...
    public void lifeStatesShouldReturnTheSameSequence() {
        ...
        Player player = new Player(id, lifeStates, directedPos, maxBombs,
                bombRange);

        Sq<LifeState> playerLifeStates = player.lifeStates();
        // We know lifeStates is not infinite
        while (!lifeStates.isEmpty()) {
            assertEquals(lifeStates.head().state(), playerLifeStates.head()
                    .state());
            assertEquals(lifeStates.head().lives(), playerLifeStates.head()
                    .lives());
            lifeStates = lifeStates.tail();
    ...
    public void lifeStateReturnTheFirstStateOfTheSequence() {
        ...
        // We know lifeStates is not infinite
        while (!lifeStates.isEmpty()) {
            LifeState playerLifeState = new Player(id, lifeStates, directedPos,
                    maxBombs, bombRange).lifeState();
    ...
    public void constructor2ReturnsPlayerInvulnerableForTicksConstantDurationFollowedByVulnerableStatesWhenLifeGreaterThanZero() {
        ...
        Sq<LifeState> invulnerable = player2.lifeStates().takeWhile(
                s -> (s.state().equals(State.INVULNERABLE))
                        && (s.lives() == lives));
        Sq<LifeState> vulnerable = player2.lifeStates().dropWhile(
                s -> (s.state().equals(State.INVULNERABLE))
                        && (s.lives() == lives));
    ...
    public void constructor2HasConstantDirectedPositionDirectedSouthLocatedAtCentralSubCellOfPosition() {
        ...
        for (int i = 0; i < THRESHOLD; ++i) {
            ...
            assertEquals(SubCell.centralSubCellOf(cellPosition), dp.head()
                    .position());
            dp = dp.tail();
        }
    }
    ...
    public void withBombRangeReturnsNewIdenticalPlayerWithNewBombRange() {
        assertEquals(newBombRange, player.withBombRange(newBombRange)
                .bombRange());
    }
    ...
    public void newDirectedPositionWithCorrectParameters() {
        DirectedPosition pos = new DirectedPosition(new SubCell(12, 7),
                Direction.E);
    ...
    public void DirectedPosition_stoppedReturnsConstantPosition() {
        DirectedPosition startingPos = new DirectedPosition(new SubCell(12, 7),
                Direction.E);
        ...
        for (int i = 0; i <= THRESHOLD; i++) {
            ...
            assertEquals("Stopped still on the same position", startingPos,
                    stopped.head());
    ...
    private void assertIsMoving(DirectedPosition startingPos) {
        ...
        for (int i = 0; i <= THRESHOLD; i++) {
            ...
            assertEquals("Direction is still the same",
                    startingPos.direction(), pos.direction());
